...
parser.add_argument("--iters1", default=2000, type=int, help="Stage 1 number of iterations")
parser.add_argument("--iters2", default=15, type=int, help="Stage 2 number of iterations")
...
parser.add_argument("--model_lr", default=1e-4, type=float, help="Model learning rate")
		...
		if INCLUDE_ALG_LOSS and e > START_ALG_L and e > 0:
			...
			if len(protected_negative) == 0 or len(nonprotected_negative) == 0:
				...
				for iterate in tqdm(range(50)):
					...
					protected_hessian += torch.eye(protected_hessian.shape[0]).cuda() * 1e-5
					...
					not_protected_hessian += torch.eye(not_protected_hessian.shape[0]).cuda() * 1e-5
